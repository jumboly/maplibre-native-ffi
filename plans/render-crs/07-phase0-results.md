...
phase0/README.md §3 の手順で確認し、結果をここに追記する:

- [ ] 都庁 IX 系: warp
      結果が地理院タイル標準地図と整合（道路中心線のズレの実測値）
- [ ] 都庁 IX 系: Mercator ベースラインとほぼ同一（γ≈0.08° → 端部変位は数 px
      以内）
- [ ] 根室 XIII 系: Mercator ベースラインに対する明確な回転（端で ≈9px）の実測
- [ ] 品質評価: リサンプラ 3 変種（bilinear / cubic / lanczos）のラベル・細線の
      滲み比較クロップと所見

## 案 C の品質所見（QGIS 検証後に記入）

（ラベルの滲み、アンチエイリアス済み細線の潰れ、Mercator 基準のままのラベル向き
などを、フェーズ 3（フック方式 PoC）の受け入れ基準として言語化する）
